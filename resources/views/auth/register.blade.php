    ...
    <div class="flex justify-center">
        ...
        <div class="w-full max-w-sm border border-slate-900 p-4 rounded-lg sm:p-6 md:p-8">
            <form class="space-y-6" action="{{ route('register.store') }}" method="POST">
                ...
                <div>
                    <label for="username" class="block mb-2 font-bold">{{ __('Username') }}</label>
                ...
                <div>
                    <label for="pin" class="block mb-2 font-bold">{{ __('Pin') }}</label>
                    <input type="password" name="pin" id="pin" placeholder="......" class="bg-transparent border rounded-lg focus:ring-pink-500 focus:border-pink-500 @error('pin') border-red-500 @enderror block w-full p-2.5 dark:placeholder-slate-400"/>
                    @error('pin')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
